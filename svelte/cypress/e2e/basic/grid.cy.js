context("Grid", () => {
	...
	it("grid in default mode with local data", () => {
		...
		cy.wxG("grid-header").children().should("have.length", 4);
		cy.get(".wx-button-expand-right").click();
		cy.get(".wx-chart").should("not.be.visible");
		cy.shot("expand-grid-in-default-mode");
		cy.get(".wx-button-expand-left").click();

		cy.get(".wx-button-expand-left").click();
		cy.wxG("grid-header").children().should("have.length", 1);
		cy.shot("expand-chart-in-default-mode");
		cy.get(".wx-button-expand-right").click();
		cy.wxG("grid-header").children().should("have.length", 4);
		...
		cy.wxG("grid-header").children().eq(2).click();
		...
		cy.wxG("grid-task-list").children().should("have.length", 20);
		cy.shot("add-task-via-right-down-corner-button-in-default-mode");
	});

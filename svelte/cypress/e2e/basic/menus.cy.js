context(
	...
	() => {
		it("default menu works with local data", () => {
			...
			cy.wxG("grid-item", 2).rightclick({ position: "center" });
			cy.wxG("menu").should("be.visible");
			cy.wxG("menu").children().should("have.length", 14);
			...
			cy.wxG("chart-item", 20).scrollIntoView().rightclick("left");
			cy.wxG("menu").should("be.visible");
			cy.wxG("menu").children().should("have.length", 14);
		...
		it("menu with conditional visibility and reduced options with local data", () => {
			...
			cy.wxG("chart-item", 1).rightclick({ force: true });
			cy.wxG("menu").should("not.exist");

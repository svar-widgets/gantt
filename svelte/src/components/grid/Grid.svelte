<script>
	...
	function init(tapi) {
		...
		tapi.intercept("collapse-column", () => {
			if (compactMode) showFullToggle = !showFullToggle;
			else
				api.exec("add-task", {
					task: {
						text: _("New Task"),
	...
	const scrollY = $derived(h < height + fullHeight);
	...
	const tableOuterStyle = $derived(
		scrollY ? tableStyle + `height:${fullHeight + height}px;` : ""
	);
	const scrollYStyle = $derived(
		scroll && showFull ? tableOuterStyle : tableStyle
	);
